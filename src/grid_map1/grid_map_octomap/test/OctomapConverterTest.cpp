...
TEST(OctomapConversion, convertOctomapToGridMap) {
  ...
  ofstream file(string("/home/rml/test_octomap.csv").c_str());
  if (file.is_open()) {
  ...
  // EXPECT_FLOAT_EQ(2.0 + 0.5 * octomap.getResolution(),
  //                 gridMap.atPosition("elevation", Position(1.0, 1.0)));
}
...
TEST(OctomapConversion, convertOctomapToGridMapWithBoundingBox) {
  ...
  EXPECT_FLOAT_EQ(octomap.getResolution(), gridmap.getResolution());
  EXPECT_FLOAT_EQ(0.5 * octomap.getResolution(),
                  gridmap.atPosition("elevation", Position(1.0, 1.0)));
}
